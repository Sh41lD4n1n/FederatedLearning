...
#------------Collect
    ...
    def handle_train(self,loss,weights,der,oasis_matrix,y_train,y_train_proba,estimated_der):
...
#------------One plot
def oneplot_comparison_array_plot(x,y_array,labels,title):
    plt.figure(figsize=(8, 8))
    for n,y in zip(labels,y_array):
...
def comparsion_array_loss_one_plot(array,n_array,gamma_array):
...
def comparsion_array_acc_one_plot(array,n_array,gamma_array):
    ...
    oneplot_comparison_array_plot(x_array,y_array,
                            labels = lables_array,title="Accuracy")
